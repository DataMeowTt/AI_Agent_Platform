    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          {totalItems >= 200 && <span className="text-amber-600">(Tối đa 200)</span>}
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-md border border-gray-200 shadow-sm space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Ngưỡng Anomaly ({rawThreshold.toFixed(2)})</label>
            <input
              type="range" min="0" max="1" step="0.01" value={rawThreshold}
              onChange={(e) => setThreshold(parseFloat(e.target.value))}
              className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
            />
            <div className="flex justify-between text-xs text-gray-400 mt-1"><span>0.5</span><span>1.0</span></div>
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => setPage(1)}
              disabled={isLoading}
              className="flex-1 h-10 px-4 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-2"
            >
              <Search className="w-4 h-4" /> Áp dụng
            </button>
            <button
              onClick={handleClearFilters}
              className="h-10 px-3 border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50"
              title="Xóa bộ lọc"
            >
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-md border border-gray-200 p-12 flex flex-col items-center text-center">
          ...
          <button onClick={handleClearFilters} className="px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-200">Đặt lại bộ lọc</button>
        </div>
      ) : (
        <>
